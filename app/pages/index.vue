...
const { data: monitoredPairs, refresh: refreshMonitored } = await useAsyncData('monitored_pairs', async () => {
...
<template>
  <div class="space-y-8">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <UCard v-for="pair in (sortedPairs?.slice(0, 3) || [])" :key="pair.id" class="shadow-sm border border-gray-100 dark:border-gray-800">
        <template #header>
        ...
        <div class="space-y-2">
          <div class="flex justify-between text-sm">
            ...
            <span class="font-medium font-mono">{{ formatPrice(getPrice(pair.coin)) }}</span>
          </div>
          <div class="flex justify-between text-sm">
            <span class="text-gray-500">Trend Since</span>
            <span>{{ formatRelativeTime(pair.last_trend_flip_daily?.since) }} ago</span>
          </div>
        </div>
        <template #footer>
          <UButton :to="`/pair/${pair.coin}`" color="neutral" variant="subtle" block>
            View Details
          </UButton>
        </template>
      </UCard>
    ...
    <UCard class="shadow-sm">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left">
          <thead class="bg-gray-50 dark:bg-gray-800 text-xs text-gray-500 uppercase font-semibold">
            <tr>
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
            <tr v-for="pair in sortedPairs" :key="pair.id" class="hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-xs">
                    {{ pair.coin[0] }}
                  </div>
                  <span class="font-semibold text-gray-900 dark:text-white">{{ pair.coin }}</span>
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <TrendIndicator :status="pair.last_trend_flip_daily?.status" :since="pair.last_trend_flip_daily?.since" />
              </td>
              <td class="px-6 py-4">
                <TrendIndicator :status="pair.last_trend_flip_weekly?.status" :since="pair.last_trend_flip_weekly?.since" />
              </td>
              ...
              <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end gap-2">
                  <UButton
                    size="xs"
                    :color="isSubscribed(pair.coin, 'D1') ? 'primary' : 'neutral'"
                    :variant="isSubscribed(pair.coin, 'D1') ? 'solid' : 'ghost'"
                    :icon="isSubscribed(pair.coin, 'D1') ? 'i-lucide-bell' : 'i-lucide-bell-off'"
                  ...
                  <UButton
                    size="xs"
                    :color="isSubscribed(pair.coin, 'W1') ? 'primary' : 'neutral'"
                    :variant="isSubscribed(pair.coin, 'W1') ? 'solid' : 'ghost'"
                    :icon="isSubscribed(pair.coin, 'W1') ? 'i-lucide-bell' : 'i-lucide-bell-off'"
                    @click="toggleSubscription(pair.coin, 'W1')"
                  >
                    W1
                  </UButton>
                  <UButton :to="`/pair/${pair.coin}`" icon="i-lucide-chevron-right" size="xs" variant="ghost" color="neutral" />
                </div>
              </td>
            </tr>
          </tbody>
