...
/**
 * MonitoredPair with its associated last trend flips joined from the trends table.
 */
export type MonitoredPairWithTrends = MonitoredPair & {
  last_trend_flip_daily: Trend | null;
  last_trend_flip_weekly: Trend | null;
};
...
export type UserSubscriptionWithDetails = UserSubscription & {
  ...
  monitored_pairs: (Pick<MonitoredPair, 'active' | 'last_trend_flip_daily_id' | 'last_trend_flip_weekly_id'> & {
    coin: string;
  }) | null;
};
