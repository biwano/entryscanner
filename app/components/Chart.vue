...
const option = computed(() => {
  ...
  const dates = props.candles.map(c => new Date(c.t).toISOString().split('T')[0]);
  const data = props.candles.map(c => [parseFloat(c.o), parseFloat(c.c), parseFloat(c.l), parseFloat(c.h)]);
